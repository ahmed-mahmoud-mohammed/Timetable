...
import halls from '../json/halls.json';

const HallsTimetable = ({ route,navigation }) => {
  color = route.params.color
    buttontxt= route.params.buttontxt
  const hallname = route.params.selectedhall;
  ...
  return (
    <ScrollView style={[styles.container,{backgroundColor:color}]}>
      <TouchableOpacity style={styles.back}><Text style={[styles.backtxt,{color:buttontxt}]}
            ...
            <Text style={[styles.title,{color:buttontxt}]}>{hallname}</Text>
      <Picker
        selectedValue={day}
      ...
      {day !== '' &&
        (halls[hallname][day] === 'No Lectures today' ? (
