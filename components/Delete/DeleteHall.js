...
const DeleteHall=({navigation,route})=>{
  color = route.params.color
    ...
    return(
        <View  style={[styles.container,{backgroundColor:color}]}>
          <TouchableOpacity style={styles.back}><Text style={[styles.backtxt,{color:buttontxt}]}
            ...
            <Text style={[styles.sign,{color:buttontxt}]}>Delete Hall</Text>
           
            <TextInput style={[styles.input,{color:buttontxt,borderBottomColor:buttontxt}]} placeholder='Hall ID' placeholderTextColor={"#9C9FA6"}
    ...
    if(id===""){
      setErr("Enter Hall ID")
    }
    else{
      const response = fetch(`https://delicate-resonance-6870.fly.dev/api/lecturehalls/${id}`, {
            method: 'Delete',
            headers: {
              'Content-Type': 'application/json',
            },
           });
      navigation.navigate("Options",{"color":color,"buttontxt":buttontxt})
